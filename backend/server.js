...
const pool = new Pool({
  connectionString: process.env.DATABASE_URL || 'postgres://appuser:example@localhost:5432/walletdb'
});
...
// register (username + password) creates user + account
app.post('/auth/register', async (req,res) => {
  ...
  try {
    ...
    // Insert user with explicit constraint name
    const r = await client.query(
      'INSERT INTO users (username, password_hash) VALUES ($1,$2) ON CONFLICT ON CONSTRAINT users_username_unique DO NOTHING RETURNING id',
      [username, hash]
    );
    ...
    // ensure account exists, using explicit constraint
    await client.query(
      'INSERT INTO accounts (user_id, balance) VALUES ($1, $2) ON CONFLICT ON CONSTRAINT accounts_user_id_key DO NOTHING RETURNING id',
      [userId, 0]
    );
...
// get account id for self
app.get('/me/account', authMiddleware, async (req,res) => {
  const r = await pool.query('SELECT id, balance FROM accounts WHERE user_id=$1', [req.user.user_id]);
...
app.post('/wallet/topup', authMiddleware, async (req,res) => {
  ...
  try {
    ...
    const acc = await client.query('SELECT id, balance FROM accounts WHERE user_id=$1 FOR UPDATE', [req.user.user_id]);
    if (acc.rowCount===0) throw new Error('account not found');
    const accountId = acc.rows[0].id;
    const newBalance = BigInt(acc.rows[0].balance) + BigInt(amount);

    const ref = uuidv4();
    const tx = await client.query('INSERT INTO transactions (reference, description) VALUES ($1,$2) RETURNING id', [ref, 'topup']);
    const txid = tx.rows[0].id;

    await client.query(
      'INSERT INTO ledger_entries (transaction_id, account_id, amount, balance_after, metadata) VALUES ($1,$2,$3,$4,$5)',
      [txid, accountId, amount, newBalance.toString(), JSON.stringify({ type: 'topup' })]
    );

    await client.query('UPDATE accounts SET balance=$1 WHERE id=$2', [newBalance.toString(), accountId]);
    await client.query('COMMIT');
...
app.post('/wallet/transfer', authMiddleware, async (req,res) => {
  ...
  if (!to_account_id || !amount || amount <= 0) return res.status(400).json({ error: 'to_account_id and positive amount required' });
  const client = await pool.connect();
  try {
    await client.query('BEGIN');

    const src = await client.query('SELECT id, balance FROM accounts WHERE user_id=$1 FOR UPDATE', [req.user.user_id]);
    if (src.rowCount===0) throw new Error('source account not found');
    ...
    const ref = uuidv4();
    const tx = await client.query('INSERT INTO transactions (reference, description) VALUES ($1,$2) RETURNING id', [ref, 'transfer']);
    const txid = tx.rows[0].id;
    ...
    await client.query('COMMIT');
    res.json({ txid, from: { id: fromId, balance: newFrom.toString() }, to: { id: to_account_id, balance: newTo.toString() } });
  } catch (err) {
    await client.query('ROLLBACK');
    console.error(err);
    res.status(500).json({ error: err.message });
...
app.get('/wallet/ledger', authMiddleware, async (req,res) => {
  const r = await pool.query(
    'SELECT le.id, le.transaction_id, le.amount, le.balance_after, le.metadata, le.created_at FROM ledger_entries le JOIN accounts a ON le.account_id=a.id WHERE a.user_id=$1 ORDER BY le.created_at DESC LIMIT 200',
    [req.user.user_id]
  );
  res.json(r.rows);
});
