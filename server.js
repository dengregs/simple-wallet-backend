...
app.post('/auth/register', async (req, res) => {
  ...
  try {
    ...
    const r = await client.query(
      ...
      'ON CONFLICT ON CONSTRAINT users_username_unique DO NOTHING RETURNING id',
      [username, hash]
    );
    ...
    await client.query(
      ...
      'ON CONFLICT ON CONSTRAINT accounts_user_id_key DO NOTHING RETURNING id',
      [userId, 0]
    );
...
app.get('/wallet/ledger', authMiddleware, async (req, res) => {
  const r = await pool.query(
    ...
    'WHERE a.user_id=$1 ORDER BY le.created_at DESC LIMIT 200',
    [req.user.user_id]
  );
